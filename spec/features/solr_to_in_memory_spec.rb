...
module Hydramata
  module Works
    ...
    describe 'A Solr object loaded into an in memory work' do
      let(:pid) { 'the_namespace:the_noid' }
      let(:work_wrangler) { SolrWrangler.new(work: work) }
      let(:work) { Work.new }

      xit 'parses the Solr object, assigning predicates and values to the work' do
        seed_predicates! do
          work_wrangler.call(pid)
      ...
      private
      def seed_predicates!(vcr_cassette = 'solr-object')

  ...
  module Works
    module WorkRunners
      describe New do
        ...
        describe Create do
          Given(:callback_config) { callback.configure(:success, :created_with_invalid_data, :failure) }
          Given(:work_type) { 'article' }
          Given(:attributes) { {} }
          Given(:runner) { described_class.new(context, &callback_config) }
        ...
        describe Edit do
          Given(:callback_config) { callback.configure(:success, :failure) }
          Given(:identifier) { '123' }
          Given(:runner) { described_class.new(context, &callback_config) }
        ...
        describe Update do
          Given(:callback_config) { callback.configure(:success, :updated_with_invalid_data, :failure) }
          Given(:work_type) { 'article' }
          Given(:attributes) { {} }
          Given(:runner) { described_class.new(context, &callback_config) }
          Given(:services) { double('Services', edit_work: returning_object, save_work: save_work_returned_value)}
          When(:result) { runner.run(work_type, attributes) }
          context 'good data' do
            Given(:save_work_returned_value) { 'valid' }
            Then { expect(result).to eq([returning_object]) }
            And { expect(callback.invoked).to eq([:success, returning_object]) }
            And { expect(services).to have_received(:edit_work).with(work_type, attributes) }
            And { expect(services).to have_received(:save_work).with(returning_object) }
          end
          context 'saved but invalid' do
            Given(:save_work_returned_value) { 'invalid' }
            Then { expect(result).to eq([returning_object]) }
            And { expect(callback.invoked).to eq([:updated_with_invalid_data, returning_object]) }
            And { expect(services).to have_received(:edit_work).with(work_type, attributes) }
            And { expect(services).to have_received(:save_work).with(returning_object) }
          end
          context 'bad data' do
